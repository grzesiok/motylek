create or replace PACKAGE BODY PKG_DOWNLOAD_FILE AS
  ...
  procedure pr_download_file(p_download_url download_file.download_url%type) AS
  ...
  BEGIN
    ...
    FROM download_file
    WHERE download_url = p_download_url;
    IF(l_url_already_downloaded > 0) THEN
      RETURN;
    END IF;
    app_logging.pkg_logging.log_job_start(i_job_name => 'odfa');
    <<try_again>>
    BEGIN
      l_download_time := systimestamp;
      l_blob := f_download(p_download_url);
    EXCEPTION
    ...
    INSERT INTO download_file(download_file_id, download_time, download_url, file_content, file_md5_sum)
      VALUES (seq_download_file.nextval, l_download_time, p_download_url, l_blob, l_hash);
    app_logging.pkg_logging.log_job_end(i_record_count => 1,
                                        i_insert_count => 1);
  END pr_download_file;
